const Doktor = require("../models/doktor.model.js");
...
exports.findAll = (req, res) => {
    ...
    Doktor.getAll(doktor, (err, data) => {
      if (err)
        res.status(500).send({
          message:
            err.message || "Some error occurred while retrieving doktors."
        });
      else res.send(data);
    });
  };
...
exports.deleteAll = (req, res) => {
    Doktor.removeAll((err, data) => {
      if (err)
        res.status(500).send({
          message:
            err.message || "Some error occurred while removing all doktors."
        });
